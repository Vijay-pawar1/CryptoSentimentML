...
RESULT_PATH = (
    ...
    / "data"
    / "processed"
    / "current_signal_date_range_results.csv"
)
...
MIN_BARS = 250
# Minimum volatility-model confidence required
MIN_CONFIDENCE = float(sys.argv[1]) if len(sys.argv) > 1 else 0.80
START_DATE = pd.Timestamp(sys.argv[2]) if len(sys.argv) > 2 else None
END_DATE = pd.Timestamp(sys.argv[3]) if len(sys.argv) > 3 else None
...
print("=" * 70)
print("BTC/USDT TRUE OOS SIGNAL ENGINE BACKTEST")
print("=" * 70)
...
print(
    f"Walk-forward start: {wf['timestamp'].iloc[0]}"
)

print(
    f"Walk-forward end:   {wf['timestamp'].iloc[-1]}"
)
...
if START_DATE is not None or END_DATE is not None:
    ...
    if START_DATE is not None:
        mask &= df_features["timestamp"] >= START_DATE
    if END_DATE is not None:
        mask &= df_features["timestamp"] <= END_DATE
    df_features = df_features.loc[mask].reset_index(drop=True)

print(f"Backtest rows: {len(df_features):,}")
if len(df_features):
    print(f"Backtest start: {df_features["timestamp"].iloc[0]}")
    print(f"Backtest end:   {df_features["timestamp"].iloc[-1]}")
...
for i in range(
    ...
    if position is not None:
        ...
        # If the position was closed on this candle,
        # do NOT open a new position until the next loop iteration.
        continue
...
print()
print("=" * 70)
print("TRUE OOS BACKTEST RESULTS")
print("=" * 70)
